...
int My_screen_width = 0;
int My_screen_height = 0;
...
void vm_main(void) {
    layer_hdl[0] = -1;
    My_screen_width = vm_graphic_get_screen_width();
    My_screen_height = vm_graphic_get_screen_height();
...
void handle_sysevt(VMINT message, VMINT param) {
    ...
    switch (message) {
        case VM_MSG_CREATE:
        case VM_MSG_ACTIVE:
            layer_hdl[0] = vm_graphic_create_layer(0, 0, My_screen_width, My_screen_height, -1);
            vm_graphic_set_clip(0, 0, My_screen_width, My_screen_height);
...
VMINT mre_read_file_display(VMWSTR path) {
    ...
    if(trigeris5 == 0) {draw_ucs2_text1();}
    ...
    int i, plotis, strng_plot, isve_i_ekr_eil_sk, nauj_strng_ilg, combUodegosIlgis, uodegosIlgis, ilgism, trigerisp;
    ...
    if (vm_file_is_eof(f_read)) {
        ...
        if (isve_i_ekr_eil_sk < 20 && strng_plot != 0 && sKonv_stringas != L'\0') {
            ...
            if (isve_i_ekr_eil_sk == 0 && wstrlen(sKonv_stringas) < 7 && dynamicPosition != 0) {
               //pageCounter = pageCounter + 1;
    ...
    if(trigerisp == 1){
      ...
      vm_graphic_line_ex(layer_hdl[0], 2, 318, 237, 318);
...
void mre_draw_black_rectangle(void) {
    ...
    //vm_graphic_fill_rect(buffer, MRE_SCREEN_START_X, MRE_SCREEN_START_Y, My_screen_width, My_screen_height, VM_COLOR_BLACK, VM_COLOR_BLACK);
    vm_graphic_fill_rect(buffer, MRE_SCREEN_START_X, MRE_SCREEN_START_Y, My_screen_width, My_screen_height, VM_COLOR_WHITE, VM_COLOR_WHITE);
    vm_graphic_flush_layer(layer_hdl, 1);
}
...
void fromArrToDisplay(void) {
    ...
    vm_graphic_textout_by_baseline(buffer, 0, g_mre_curr_y, s, wstrlen(s), VM_COLOR_BLACK, 12);
    //vm_graphic_textout_by_baseline(buffer, 0, g_mre_curr_y, s, wstrlen(s), VM_COLOR_WHITE, 12);
...
void draw_ucs2_text1(void) {

    VMCHAR text3[100] = "Loading...";
    VMWCHAR text1[100];
    int font_height = 16;
    int wstr_len;
    int x;
    ...
    wstr_len = vm_graphic_get_string_width(text1);
    x = (My_screen_width - wstr_len) / 2;
    y = (My_screen_height / 2) - font_height;

    //buffer = vm_graphic_get_layer_buffer(layer_hdl[0]);

    vm_graphic_fill_rect(buffer, 0, 0, My_screen_width, My_screen_height, VM_COLOR_WHITE, VM_COLOR_WHITE);
    //vm_graphic_fill_rect(buffer, 0, 0, My_screen_width, My_screen_height, VM_COLOR_BLACK, VM_COLOR_BLACK);

    vm_graphic_textout(buffer, x, y, text1, vm_wstrlen(text1), VM_COLOR_BLACK);
    //vm_graphic_textout(buffer, x, y, text1, vm_wstrlen(text1), VM_COLOR_WHITE);
